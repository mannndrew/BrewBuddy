...
void setPinAuxFunction(PORT port, uint8_t pin, uint32_t fn)
{
    ...
    // make sure the value is the correct mask header
        fn = fn & (0x0000000F << (pin*4));
    switch(port)
    {
        // erase out bits then or it
        case PORTA:
...
//pin mask is a num between 0 and 15, tries to keep you from damaging other pins
void selectPinInterruptRisingEdge(PORT port, uint8_t pin)
{
    uint32_t *p;
    p = (uint32_t *)port + pin + OFS_DATA_TO_IS;
    *p = 0;
    // rising edge, not both edges and rising
...
void selectPinInterruptLowLevel(PORT port, uint8_t pin) // low levels and using levels
{
    uint32_t *p;
    p = (uint32_t *)port + pin + OFS_DATA_TO_IS;
    *p = 1;
